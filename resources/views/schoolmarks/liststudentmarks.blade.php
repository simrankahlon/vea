...
<ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">List Student School Marks</li>
</ol>
@endsection
@section('content')
<div class="card">
    <div class="card-block">
        <div class="row">
            ...
            <div class="col-xs-6">
                ...
                @endphp
                <form action="{{ url('/schoolmarks/'.$marks->id.'/'.$batch->id.'/'.$standard.'/filters') }}" method="get" id="marks_filter">
                    {{ csrf_field() }}
                                            ...
                                            <tbody id="tasks-list">
                                             @foreach ($admission as $adm)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div>
                                                            @if($adm->schoolmarks_obtained!=null)
                                                                @if($adm->schoolmarks_obtained < $adm->passing_marks)
                                                                    <strong style="color:red">{{$adm->schoolmarks_obtained}}</strong>
                                                                @else
                                                                    <strong>{{$adm->schoolmarks_obtained}}</strong></div>
                                                                @endif
                                                            @else
